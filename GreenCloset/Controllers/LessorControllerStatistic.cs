...
namespace GreenCloset.Controllers
{
    public partial class LessorController : BaseController
    {
        ...
        public IActionResult Statistic()
        {
            ...
            if (!string.IsNullOrEmpty(emailUser))
            {
                ViewBag.TotalProduct = _facadeService
                    .Product.GetLessorProducts(emailUser)
                    .Where(p => p.Available)
                    .Count();
            ...
            var ownerEmail = HttpContext.User.FindFirst(ClaimTypes.Email)?.Value;
            var allOrders = _facadeService.Order.GetOrdersByProductOwner(ownerEmail).ToList();
        ...
        public IActionResult GetMonthlyProfit(int year)
        {
            ...
            {
                Console.WriteLine(
                    $"OrderId: {order.Id}, Month: {order.OrderDate.Month}, Price: {order.TotalPrice}, User: {order.UserId}"
                );
